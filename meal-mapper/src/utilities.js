export function businessIcon(business) {
  ...
  var item =
    cuisine.indexOf(",") > 0
      ? cuisine.toLowerCase().substr(0, cuisine.indexOf(",")).trim()
...
export function getAddress(marker) {
  var address =
    marker.gsx$address !== undefined && marker.gsx$address.$t !== ""
      ? marker.gsx$address.$t + ", "
      : "";
  address =
    address +
    (marker.gsx$city !== undefined && marker.gsx$city.$t !== ""
      ? marker.gsx$city.$t + ", "
      : "");
  address =
    address +
    (marker.gsx$state !== undefined && marker.gsx$state.$t !== ""
      ? marker.gsx$state.$t + " "
      : "");
  return (
    address +
    (marker.gsx$zip !== undefined && marker.gsx$zip.$t !== ""
      ? marker.gsx$zip.$t
      : "")
  );
}
